...
use crate::{FileSystemClient, DeltaResult, FileMeta, FileSlice, Error};
...
impl FileSystemClient for SimpleFilesystemClient {
    ...
    fn list_from(&self, path: &Url) -> DeltaResult<Box<dyn Iterator<Item = DeltaResult<FileMeta>>>> {
        if path.scheme() == "file" {
            let path = path.path();
            let last_slash = path.rfind('/').ok_or(Error::Generic(format!("Invalid path for list_from: {}", path)))?;
            let all_ents: std::io::Result<Vec<DirEntry>> = std::fs::read_dir(&path[0..last_slash])?.sorted_by_key(|ent_res| {
                ent_res.as_ref().map(|ent| ent.path()).unwrap_or_else(|_| PathBuf::new())
            }).collect();
            ...
            let it = all_ents.into_iter().map(|ent| {
                ent.metadata().map_err(|e| Error::IOError(e)).and_then(|metadata| {
                    let last_modified: u64 = metadata.modified().map(|modified| {
                    ...
                    println!("Adding {:#?}", ent);
                    Url::from_file_path(ent.path()).map(|location| {
                        FileMeta {
                            location,
